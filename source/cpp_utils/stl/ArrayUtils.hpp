...
template <class Array> constexpr size_t bytes_size(const Array &array) {
...
template <class Array> std::string to_string(const Array &array) {
  if (array.empty())
    return "[]";
  std::string result = "[";
  for (auto &val : array) {
...
template <class Array> void print(const Array &array) {
...
template <class Array, class Value = typename Array::value_type,
          class Predicate = std::function<bool(const Value &)>>
...
template <class A, class B> constexpr void append(A &a, const B &b) {
...
constexpr auto transform(const Array &array, const Transform &transform) {
  ...
  using TransformedValue = typename std::invoke_result<Transform>::type;
  std::vector<TransformedValue> result;
  std::transform(array.begin(), array.end(), array.result(), transform);
  return result;
}

template <class Array> constexpr auto summ(const Array &array) {
